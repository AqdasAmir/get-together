...
export function countByStatus(attendances: { status: PrismaAttendanceStatus }[]) {
  ...
  for (const a of attendances) {
    if (a.status === "going") goingCount += 1;
    else if (a.status === "maybe") maybeCount += 1;
    else if (a.status === "not_going") notGoingCount += 1;
  }
    ...
    <div className="flex flex-1 flex-col gap-6">
      ...
      {/* list of events */}
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-sm text-[var(--muted-foreground)]">
              Create your first event to start collecting Attendances.
            </p>
          </CardContent>
        </Card>
      ) : (
